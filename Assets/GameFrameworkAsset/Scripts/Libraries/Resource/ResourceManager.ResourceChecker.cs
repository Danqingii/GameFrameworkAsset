        ...
        private sealed partial class ResourceChecker
        {
            ...
            private void RefreshCheckInfoStatus()
            {
                ...
                foreach (KeyValuePair<ResourceName, CheckInfo> checkInfo in m_CheckInfos)
                {
                    ...
                    else if (ci.Status == CheckInfo.CheckStatus.StorageInReadWrite)
                    {
                        if (ci.NeedMoveToDisk || ci.NeedMoveToFileSystem)
                        {
                            ...
                            if (ci.NeedMoveToFileSystem)
                            {
                                ...
                                {
                                    throw new GameFrameworkException($"Write resource '{resourceFullName}' to file system '{fileSystem.FullPath}' error.");
                                }
                    ...
                    {
                        throw new GameFrameworkException($"Check resources '{ci.ResourceName.FullName}' error with unknown status.");
                    }
            ...
            private void OnLoadUpdatableVersionListSuccess(string fileUri, byte[] bytes, float duration, object userData)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    throw new GameFrameworkException($"Parse updatable version list exception '{exception}'.", exception);
                }
                finally
                {
            ...
            {
                throw new GameFrameworkException($"Updatable version list '{fileUri}' is invalid, error message is '{(string.IsNullOrEmpty(errorMessage) ? "<Empty>" : errorMessage)}'.");
            }
            ...
            private void OnLoadReadOnlyVersionListSuccess(string fileUri, byte[] bytes, float duration, object userData)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    throw new GameFrameworkException($"Parse read-only version list exception '{exception}'.", exception);
                }
                finally
                {
                    if (memoryStream != null)
            ...
            private void OnLoadReadWriteVersionListSuccess(string fileUri, byte[] bytes, float duration, object userData)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    throw new GameFrameworkException($"Parse read-write version list exception '{exception}'.", exception);
                }
                finally
                {
                    if (memoryStream != null)
