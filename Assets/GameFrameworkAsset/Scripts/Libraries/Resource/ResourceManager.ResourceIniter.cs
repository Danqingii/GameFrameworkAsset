        ...
        private sealed class ResourceIniter
        {
            ...
            private void OnLoadPackageVersionListSuccess(string fileUri, byte[] bytes, float duration, object userData)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    throw new GameFrameworkException($"Parse package version list exception '{exception}'.", exception);
                }
                finally
                {
            ...
            {
                throw new GameFrameworkException($"Package version list '{fileUri}' is invalid, error message is '{(string.IsNullOrEmpty(errorMessage) ? "<Empty>" : errorMessage)}'.");
            }
        }
    }
}
