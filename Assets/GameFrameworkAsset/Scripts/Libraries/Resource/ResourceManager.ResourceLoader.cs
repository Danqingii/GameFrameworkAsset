    ...
    internal sealed partial class ResourceManager : GameFrameworkModule, IResourceManager
    {
        ...
        private sealed partial class ResourceLoader
        {
            ...
            public void LoadAsset(string assetName, Type assetType, int priority, LoadAssetCallbacks loadAssetCallbacks, object userData)
            {
                ...
                {
                    string errorMessage = $"Can not load asset '{assetName}'.";
                ...
                {
                    string errorMessage = $"Can not load asset '{assetName}' which is a binary asset.";
                    if (loadAssetCallbacks.LoadAssetFailureCallback != null)
                    {
                ...
                foreach (string dependencyAssetName in dependencyAssetNames)
                {
                    ...
                    {
                        string errorMessage = $"Can not load dependency asset '{dependencyAssetName}' when load asset '{assetName}'.";
                        if (loadAssetCallbacks.LoadAssetFailureCallback != null)
                        {
            ...
            public void LoadScene(string sceneAssetName, int priority, LoadSceneCallbacks loadSceneCallbacks, object userData)
            {
                ...
                {
                    string errorMessage = $"Can not load scene '{sceneAssetName}'.";
                ...
                if (resourceInfo.IsLoadFromBinary)
                {
                    string errorMessage = $"Can not load scene asset '{sceneAssetName}' which is a binary asset.";
                    if (loadSceneCallbacks.LoadSceneFailureCallback != null)
                    {
                ...
                foreach (string dependencyAssetName in dependencyAssetNames)
                {
                    if (!LoadDependencyAsset(dependencyAssetName, priority, mainTask, userData))
                    {
                        string errorMessage = $"Can not load dependency asset '{dependencyAssetName}' when load scene '{sceneAssetName}'.";
                        if (loadSceneCallbacks.LoadSceneFailureCallback != null)
                        {
            ...
            public void UnloadScene(string sceneAssetName, UnloadSceneCallbacks unloadSceneCallbacks, object userData)
            {
                ...
                {
                    throw new GameFrameworkException($"Can not find asset of scene '{sceneAssetName}'.");
                }
            ...
            public void LoadBinary(string binaryAssetName, LoadBinaryCallbacks loadBinaryCallbacks, object userData)
            {
                ...
                if (resourceInfo == null)
                {
                    string errorMessage = $"Can not load binary '{binaryAssetName}' which is not exist.";
                ...
                if (!resourceInfo.Ready)
                {
                    string errorMessage = $"Can not load binary '{binaryAssetName}' which is not ready.";
                    if (loadBinaryCallbacks.LoadBinaryFailureCallback != null)
                    {
                ...
                if (!resourceInfo.IsLoadFromBinary)
                {
                    string errorMessage = $"Can not load binary '{binaryAssetName}' which is not a binary asset.";
                    if (loadBinaryCallbacks.LoadBinaryFailureCallback != null)
                    {
            ...
            public byte[] LoadBinaryFromFileSystem(string binaryAssetName)
            {
                ...
                if (resourceInfo == null)
                {
                    throw new GameFrameworkException($"Can not load binary '{binaryAssetName}' from file system which is not exist.");
                }

                if (!resourceInfo.Ready)
                {
                    throw new GameFrameworkException($"Can not load binary '{binaryAssetName}' from file system which is not ready.");
                }

                if (!resourceInfo.IsLoadFromBinary)
                {
                    throw new GameFrameworkException($"Can not load binary '{binaryAssetName}' from file system which is not a binary asset.");
                }

                if (!resourceInfo.UseFileSystem)
                {
                    throw new GameFrameworkException($"Can not load binary '{binaryAssetName}' from file system which is not use file system.");
                }
