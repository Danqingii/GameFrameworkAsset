    ...
    internal sealed partial class ResourceManager : GameFrameworkModule, IResourceManager
    {
        ...
        private sealed class VersionListProcessor
        {
            ...
            public void UpdateVersionList(int versionListLength, int versionListHashCode, int versionListCompressedLength, int versionListCompressedHashCode)
            {
                ...
                string latestVersionListFullNameWithCrc32 = $"{RemoteVersionListFileName.Substring(0, dotPosition)}.{(m_VersionListHashCode):x8}.{RemoteVersionListFileName.Substring(dotPosition + 1)}";
            ...
            private void OnDownloadSuccess(object sender, DownloadSuccessEventArgs e)
            {
                ...
                try
                {
                    using (FileStream fileStream = new FileStream(e.DownloadPath, FileMode.Open, FileAccess.ReadWrite))
                    {
                        ...
                        {
                            fileStream.Close();
                            string errorMessage = $"Latest version list compressed length error, need '{m_VersionListCompressedLength}', downloaded '{length}'.";
                        ...
                        {
                            fileStream.Close();
                            string errorMessage = $"Latest version list compressed hash code error, need '{m_VersionListCompressedHashCode}', downloaded '{hashCode}'.";
                            DownloadFailureEventArgs downloadFailureEventArgs = DownloadFailureEventArgs.Create(e.SerialId, e.DownloadPath, e.DownloadUri, errorMessage, e.UserData);
                            OnDownloadFailure(this, downloadFailureEventArgs);
                            ReferencePool.Release(downloadFailureEventArgs);
                            return;
                        ...
                        {
                            fileStream.Close();
                            string errorMessage = $"Unable to decompress latest version list '{e.DownloadPath}'.";
                            DownloadFailureEventArgs downloadFailureEventArgs = DownloadFailureEventArgs.Create(e.SerialId, e.DownloadPath, e.DownloadUri, errorMessage, e.UserData);
                            OnDownloadFailure(this, downloadFailureEventArgs);
                            ReferencePool.Release(downloadFailureEventArgs);
                            return;
                        ...
                        {
                            fileStream.Close();
                            string errorMessage = $"Latest version list length error, need '{m_VersionListLength}', downloaded '{uncompressedLength}'.";
                            DownloadFailureEventArgs downloadFailureEventArgs = DownloadFailureEventArgs.Create(e.SerialId, e.DownloadPath, e.DownloadUri, errorMessage, e.UserData);
                            OnDownloadFailure(this, downloadFailureEventArgs);
                            ReferencePool.Release(downloadFailureEventArgs);
                            return;
                ...
                {
                    string errorMessage = $"Update latest version list '{e.DownloadPath}' with error message '{exception}'.";
                    DownloadFailureEventArgs downloadFailureEventArgs = DownloadFailureEventArgs.Create(e.SerialId, e.DownloadPath, e.DownloadUri, errorMessage, e.UserData);
                    OnDownloadFailure(this, downloadFailureEventArgs);
                    ReferencePool.Release(downloadFailureEventArgs);
                }
