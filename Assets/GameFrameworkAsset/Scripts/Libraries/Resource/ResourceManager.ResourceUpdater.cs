    ...
    internal sealed partial class ResourceManager : GameFrameworkModule, IResourceManager
    {
        ...
        private sealed partial class ResourceUpdater
        {
            ...
            public ResourceUpdater(ResourceManager resourceManager)
            {
                ...
                m_ReadWriteVersionListTempFileName = $"{m_ReadWriteVersionListFileName}.{TempExtension}";
            ...
            public void ApplyResources(string resourcePackPath)
            {
                ...
                if (m_ApplyingResourcePackStream != null)
                {
                    throw new GameFrameworkException($"There is already a resource pack '{m_ApplyingResourcePackPath}' being applied.");
                }

                if (m_UpdatingResourceGroup != null)
                {
                    throw new GameFrameworkException($"There is already a resource group '{m_UpdatingResourceGroup.Name}' being updated.");
                }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new GameFrameworkException($"Apply resources '{resourcePackPath}' with exception '{exception}'.", exception);
                }
            }
                ...
                try
                {
                    ...
                    using (FileStream fileStream = new FileStream(applyInfo.ResourcePath, FileMode.Create, FileAccess.ReadWrite))
                    {
                        ...
                        if (compressed)
                        {
                            ...
                            if (hashCode != applyInfo.CompressedHashCode)
                            {
                                ...
                                {
                                    string errorMessage = $"Resource compressed hash code error, need '{applyInfo.CompressedHashCode}', applied '{hashCode}'.";
                            ...
                            if (!Utility.Compression.Decompress(fileStream, m_ResourceManager.m_CachedStream))
                            {
                                if (ResourceApplyFailure != null)
                                {
                                    string errorMessage = $"Unable to decompress resource '{applyInfo.ResourcePath}'.";
                                    ResourceApplyFailure(applyInfo.ResourceName, m_ApplyingResourcePackPath, errorMessage);
                                }
                        ...
                        else
                        {
                            ...
                            if (hashCode != applyInfo.HashCode)
                            {
                                if (ResourceApplyFailure != null)
                                {
                                    string errorMessage = $"Resource hash code error, need '{applyInfo.HashCode}', applied '{hashCode}'.";
                                    ResourceApplyFailure(applyInfo.ResourceName, m_ApplyingResourcePackPath, errorMessage);
                                }
                    ...
                    if (applyInfo.UseFileSystem)
                    {
                        ...
                        if (!retVal)
                        {
                            if (ResourceApplyFailure != null)
                            {
                                string errorMessage = $"Unable to write resource '{applyInfo.ResourcePath}' to file system '{applyInfo.FileSystemName}'.";
                                ResourceApplyFailure(applyInfo.ResourceName, m_ApplyingResourcePackPath, errorMessage);
                            }
                    ...
                    string downloadingResource = $"{applyInfo.ResourcePath}.download";
            ...
            private bool DownloadResource(UpdateInfo updateInfo)
            {
                ...
                string resourceFullNameWithCrc32 = updateInfo.ResourceName.Variant != null ? $"{updateInfo.ResourceName.Name}.{updateInfo.ResourceName.Variant}.{(updateInfo.HashCode):x8}.{DefaultExtension}" : $"{updateInfo.ResourceName.Name}.{(updateInfo.HashCode):x8}.{DefaultExtension}";
            ...
            private void GenerateReadWriteVersionList()
            {
                ...
                catch (Exception exception)
                {
                    ...
                    throw new GameFrameworkException($"Generate read-write version list exception '{exception}'.", exception);
                }
            ...
            private void OnDownloadStart(object sender, DownloadStartEventArgs e)
            {
                ...
                {
                    throw new GameFrameworkException($"File '{e.DownloadPath}' is too large.");
                }
            ...
            private void OnDownloadUpdate(object sender, DownloadUpdateEventArgs e)
            {
                ...
                if (e.CurrentLength > updateInfo.CompressedLength)
                {
                    ...
                    string downloadFile = $"{e.DownloadPath}.download";
                    if (File.Exists(downloadFile))
                    {
                        File.Delete(downloadFile);
                    }

                    string errorMessage = $"When download update, downloaded length is larger than compressed length, need '{updateInfo.CompressedLength}', downloaded '{e.CurrentLength}'.";
            ...
            private void OnDownloadSuccess(object sender, DownloadSuccessEventArgs e)
            {
                ...
                try
                {
                    using (FileStream fileStream = new FileStream(e.DownloadPath, FileMode.Open, FileAccess.ReadWrite))
                    {
                        ...
                        {
                            fileStream.Close();
                            string errorMessage = $"Resource compressed length error, need '{updateInfo.CompressedLength}', downloaded '{length}'.";
                            DownloadFailureEventArgs downloadFailureEventArgs = DownloadFailureEventArgs.Create(e.SerialId, e.DownloadPath, e.DownloadUri, errorMessage, e.UserData);
                            OnDownloadFailure(this, downloadFailureEventArgs);
                            ReferencePool.Release(downloadFailureEventArgs);
                            return;
                        ...
                        if (compressed)
                        {
                            ...
                            {
                                fileStream.Close();
                                string errorMessage = $"Resource compressed hash code error, need '{updateInfo.CompressedHashCode}', downloaded '{hashCode}'.";
                                DownloadFailureEventArgs downloadFailureEventArgs = DownloadFailureEventArgs.Create(e.SerialId, e.DownloadPath, e.DownloadUri, errorMessage, e.UserData);
                                OnDownloadFailure(this, downloadFailureEventArgs);
                                ReferencePool.Release(downloadFailureEventArgs);
                                return;
                            ...
                            {
                                fileStream.Close();
                                string errorMessage = $"Unable to decompress resource '{e.DownloadPath}'.";
                                DownloadFailureEventArgs downloadFailureEventArgs = DownloadFailureEventArgs.Create(e.SerialId, e.DownloadPath, e.DownloadUri, errorMessage, e.UserData);
                                OnDownloadFailure(this, downloadFailureEventArgs);
                                ReferencePool.Release(downloadFailureEventArgs);
                                return;
                            ...
                            {
                                fileStream.Close();
                                string errorMessage = $"Resource length error, need '{updateInfo.Length}', downloaded '{uncompressedLength}'.";
                                DownloadFailureEventArgs downloadFailureEventArgs = DownloadFailureEventArgs.Create(e.SerialId, e.DownloadPath, e.DownloadUri, errorMessage, e.UserData);
                                OnDownloadFailure(this, downloadFailureEventArgs);
                                ReferencePool.Release(downloadFailureEventArgs);
                                return;
                        ...
                        else
                        {
                            ...
                            {
                                fileStream.Close();
                                string errorMessage = $"Resource hash code error, need '{updateInfo.HashCode}', downloaded '{hashCode}'.";
                                DownloadFailureEventArgs downloadFailureEventArgs = DownloadFailureEventArgs.Create(e.SerialId, e.DownloadPath, e.DownloadUri, errorMessage, e.UserData);
                                OnDownloadFailure(this, downloadFailureEventArgs);
                                ReferencePool.Release(downloadFailureEventArgs);
                                return;
                    ...
                    if (updateInfo.UseFileSystem)
                    {
                        ...
                        if (!retVal)
                        {
                            string errorMessage = $"Write resource to file system '{fileSystem.FullPath}' error.";
                            DownloadFailureEventArgs downloadFailureEventArgs = DownloadFailureEventArgs.Create(e.SerialId, e.DownloadPath, e.DownloadUri, errorMessage, e.UserData);
                            OnDownloadFailure(this, downloadFailureEventArgs);
                            ReferencePool.Release(downloadFailureEventArgs);
                            return;
                ...
                {
                    string errorMessage = $"Update resource '{e.DownloadPath}' with error message '{exception}'.";
                    DownloadFailureEventArgs downloadFailureEventArgs = DownloadFailureEventArgs.Create(e.SerialId, e.DownloadPath, e.DownloadUri, errorMessage, e.UserData);
                    OnDownloadFailure(this, downloadFailureEventArgs);
                    ReferencePool.Release(downloadFailureEventArgs);
                }
